...
\centering
\begin{tikzpicture}
\draw[fill=black] (0,0) circle (0.06) node[below=4pt] {agent};
\draw (0,0) circle (1.1);
\draw (0,0) circle (2.0);
\draw (0,0) circle (3.0);
...
\node[draw,circle,minimum size=1.3cm,align=center] (phi)  {$\Phi$};
\node[draw,circle,minimum size=1.3cm,align=center] (temp) [right of=phi]    {$T^*$};
\node[draw,circle,minimum size=1.3cm,align=center] (S)    [right of=temp]   {$S$};
\node[draw,circle,minimum size=1.3cm,align=center] (CT)   [right of=S]      {$C_T$};
\draw[->] (phi) -- (temp) node[midway,above] {\small $|\nabla\Phi|$};
\draw[->] (temp) -- (S)   node[midway,above] {\small $\alpha(\cdot)^2$};
\draw[->] (S) -- (CT)     node[midway,above] {\small $\tfrac{1}{T^*S}$};
\draw[->,bend left=40] (phi) to node[above] {\small direct coupling} (S);
\end{tikzpicture}
