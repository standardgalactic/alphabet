...
% \leadsto is an amssymb convenience alias for \rightsquigarrow. This
% document uses unicode-math instead of amssymb, so the alias was never
% defined even though \rightsquigarrow itself works (unicode-math provides
% it directly via the font's Unicode table). Every bare \leadsto in the
% prose below depends on this line.
\newcommand{\leadsto}{\rightsquigarrow}

% \dashrightarrow is likewise an amssymb-only name. Unlike \rightsquigarrow,
% unicode-math does not appear to resolve it against the loaded math font
% (TeX Gyre Pagella Math may simply lack the glyph), so it is built here
% from primitives guaranteed to exist in any math font, rather than by
% aliasing to a Unicode character whose availability we cannot confirm.
\newcommand{\dashrightarrow}{\mathrel{{-}\,{-}\,{\to}}}

% \Rel, \Ob, \Continuation: three more operator-style names, all missed by
% an earlier automated sweep of this document because each is always
% followed immediately by a subscript (\Rel_t, \Ob(...), \Continuation_{...})
% with no separating space -- which defeats a naive \b-word-boundary
% search, since underscore counts as a word character. Found only once an
% actual compile was attempted.
\newcommand{\Rel}{\operatorname{Rel}}
\newcommand{\Ob}{\operatorname{Ob}}
\newcommand{\Continuation}{\operatorname{Continuation}}
...
% KOMA-Script (scrbase) only defines \appendixname once its language
% strings are activated at \begin{document}, so redefining it here
% directly would fail. Deferred instead of relocating this block.
\AtBeginDocument{\renewcommand{\appendixname}{Technical Appendix}}
...
\[
  \rho
  =
  \text{\enquote{the occurrence of \(\varphi\) is generation zero}}.
\]
